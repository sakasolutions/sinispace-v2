  ...
  return (
    <div className="flex h-[100dvh] overflow-x-hidden relative">
      ...
      <main className="relative z-10 flex-1 md:ml-64 flex flex-col overflow-hidden" style={{ height: '100%', maxHeight: '100dvh', overflowY: 'hidden', overflowX: 'hidden' } as React.CSSProperties}>
        ...
        >
          <div className="h-full [&>*[data-no-padding]]:h-full [&>*:not([data-no-padding])]:min-h-full [&>*:not([data-no-padding])]:py-3 [&>*:not([data-no-padding])]:px-0 [&>*:not([data-no-padding])]:sm:p-4 [&>*:not([data-no-padding])]:md:p-6 [&>*:not([data-no-padding])]:lg:p-8 -mt-[env(safe-area-inset-top)] md:mt-0">
            {children}
          </div>
        </div>
      </main>
