      ...
      {filteredTools.length > 0 ? (
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
          {filteredTools.map((tool) => {
            ...
            const cardClassName = `group relative flex flex-col h-full min-h-[180px] p-5 rounded-2xl border backdrop-blur-xl transition-all duration-500 ease-out hover:-translate-y-1 hover:shadow-lg overflow-hidden transform-gpu backface-hidden ${cardColors.bg} ${cardColors.border} ${cardColors.hoverBorder} ${cardColors.hoverShadow} ${tool.available ? 'cursor-pointer' : 'opacity-75 cursor-not-allowed'}`;
            ...
            const cardContent = (
              <>
                ...
                <div
                  className={`absolute -bottom-4 -right-4 w-24 h-24 rounded-full blur-2xl opacity-15 ${glowClass} pointer-events-none`}
                  style={{ zIndex: 0 }}
                />
              </>
            );
